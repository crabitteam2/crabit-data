...
백엔드 연동용 무상태 HTTP 서비스는 `CRABIT_RECAP_TOKEN=... python -m recap_service`로 실행합니다. 정식 내부 계약과 제한·오류·재시도 경계는 [`api/recap-generation-v1.yaml`](api/recap-generation-v1.yaml)에 있습니다. Spring은 스냅샷·생성 ID·재시도·저장을 소유하고 Python은 상태를 저장하지 않은 채 고정된 입력을 동기 계산합니다.
